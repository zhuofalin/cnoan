...
import utils
# from .an2cn import CN2ArabicNumerals
from an2cn import ArabicNumerals2CN
from base import ConvertBase


class CN2ArabicNumerals(ConvertBase):
    """
    ...
    def __init__(self) -> None:
        super(CN2ArabicNumerals, self).__init__()
        ...
        self.ac = ArabicNumerals2CN()
        self.mode_list = ['strict', 'normal', 'smart']

    def convert(self, inputs: Union[str, int, float] = None, mode: str = "strict") -> Union[float, int]:
        """
        Convert Chinese numerals to Arabic numerals

        Function description:
            1. In strict mode (default), only those that strictly conform to the spelling of numbers can be converted
            2. In normal mode, one, two, three can be converted
            3. In smart mode, the mixed spelling of 123 can be converted
            4. The above three modes all support negative numbers and decimals

        :param inputs: Chinese numerals
        :param mode: strict,normal,smart
    ...
    def __copy_num(self, num):
        cn_num = ""
        for n in num:
            cn_num += self.conf["number_lower_an2cn"][int(n)]
        return cn_num
    ...
    def __check_input_data_is_valid(self, check_data: str, mode: str) -> (int, str, str, bool):
        ...
        if "点" in check_data:
            ...
            if mode == "smart":
                ...
                integer_data = re.sub(r"\d+", lambda x: self.ac.convert(x.group()), integer_data)
                mode = "normal"
            ...
            if mode == "strict":
                ...
                if result_speaking_mode:
                    if result_speaking_mode.group() == integer_data:
                        _unit = self.conf["unit_lower_an2cn"][self.conf["unit_cn2an"][integer_data[1]] // 10]
    ...
    def __integer_convert(self, integer_data: str) -> int:
        ...
        # return format(int(output_integer), ",")
...
if __name__ == '__main__':
    cn2an = CN2ArabicNumerals()
    # print(cn2an.convert('2千', mode='strict'))
    # print(cn2an.convert('2千', mode='normal'))
    print(cn2an.convert('2千', mode='smart'))
